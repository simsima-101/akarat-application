...
//       handoverQuarter: null,
//       handoverYear: null,
//       isLoading: true, // Show loading during fetch
//     ));
//
//     final fetchedAmenities = await _repository.fetchAmenities();
//
...
//       fetchedPropertyType = await _repository.fetchPropertyTypes(type);
//
//       // Auto-select subtype if passed from home (e.g., 'Villa', 'Apartment')
//       if (event.propertyCategoryType != null &&
//           fetchedPropertyType?.data != null) {
//         final target = event.propertyCategoryType!.trim().toLowerCase();
//         final index = fetchedPropertyType!.data!.indexWhere(
//           (item) => item.name?.trim().toLowerCase() == target,
//         );
//         if (index != -1) {
//           emit(state.copyWith(
//             selectedtype: index,
//             property_type: fetchedPropertyType.data![index].name ?? '',
//           ));
//         }
//       }
//     }
...
//     ));
//
//     // Capture initial snapshot so hasChanges works from the start
//     // → Reset button enables when user makes changes
//     add(const FilterCaptureInitialSnapshot());
//   }
//
...
//     String? newOption = '';
//
//     PropertyTypeModel? fetchedModel;
//
//     // Match old Provider logic + await fetch where needed
//     if (cat == 0) {
//       // Rent general
//       prodIndex = 1;
//       newPurpose = product[prodIndex];
...
//       newPurpose = product[prodIndex];
//       propType = 1;
//       // Await fetch like old code
//       fetchedModel = await _repository.fetchPropertyTypes('Commercial');
//       completionIndex = 0;
//       newOption = '';
//     } else if (cat == 4 || cat == 5) {
//       // Villas or Apartment
...
//       propType = 0;
//       // Await fetch
//       fetchedModel = await _repository.fetchPropertyTypes('Residential');
//
//       // Pre-select sub-type (Villa or Apartment) after fetch
//       String targetCategory = cat == 4 ? 'Villa' : 'Apartment';
//       final targetLower = targetCategory.trim().toLowerCase();
//       final index = fetchedModel?.data?.indexWhere(
//             (item) => item.name?.trim().toLowerCase() == targetLower,
//           ) ??
//           -1;
//       if (index != -1 && fetchedModel?.data != null) {
//         selectedtype = index;
//         property_type = fetchedModel!.data![index].name ?? '';
//       }
//       completionIndex = 0;
//       newOption = '';
//     }
...
//       selectedPropType: propType,
//       propertyTypeModel:
//           fetchedModel ?? state.propertyTypeModel, // Use fetched if available
//       isResetLoading: false,
//     ));
//
//     // Clear locations
//     event.context.read<LocationPickerProvider>().selectedLocationList.clear();
//
//     // Capture snapshot from FINAL state
//     add(const FilterCaptureInitialSnapshot());
//
//     if (event.isUpdate) {
//       add(FilterUpdateFilterCount(context: event.context));
...
//   //     FilterShowResult event, Emitter<FilterState> emit) async {
//   //   emit(state.copyWith(isLoading: true));
//   //
//   //   add(FilterUpdateFilterCount(context: event.context));
//   //
...
//
//   Future<void> _onShowResult(
//     FilterShowResult event,
//     Emitter<FilterState> emit,
//   ) async {
//     // Show loading spinner in the filter screen while we wait
//     emit(state.copyWith(isLoading: true));
//
//     // Wait for the actual fetch to complete (this is the key part)
//     await _onUpdateFilterCount(
//       FilterUpdateFilterCount(
//         context: event.context,
//         loadMore: false, // fresh results, not loading more
//       ),
//       emit,
//     );
//
//     // Now the state has real data → safe to decide what to do next
//     emit(state.copyWith(isLoading: false));
//
//     if (!event.autoUpdate) {
//       // Check the real result after fetch finished
//       if (state.filterModel?.data?.isEmpty ?? true) {
//         // No properties at all → show your nice "No Property Found" dialog
//         event.onFilterResultZero?.call();
//       } else {
//         // Has properties → go to the results list screen
//         event.onFilterResultNotZero?.call();
//       }
//     }
//   }
//
