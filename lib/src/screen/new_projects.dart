...
import '../features/property/data/models/project_model.dart';
  import '../features/property/presentation/bloc/favorite_bloc.dart';     // ← Bloc for favorites
...
import '../features/property/presentation/bloc/new_projects_bloc.dart';
  ...
  class NewProjectsScreen extends StatelessWidget {
    ...
    Future<bool> markAsContacted(BuildContext context, int propertyId, {required String contactType}) async {
      ...
      try {
        // ────────────────────────────────────────────────
        // Same language detection logic as PropertyRepository & PropertyCard
        // ────────────────────────────────────────────────
        ...
        try {
          ...
          debugPrint('Warning (markAsContacted in NewProjectsScreen): Could not read window.locale → fallback to en');
        }
        ...
        debugPrint('→ NewProjects markAsContacted → Accept-Language: $acceptLanguage (lang: $langCode)');
        ...
        child: Scaffold(
          ...
          body: BlocBuilder<NewProjectsBloc, NewProjectsState>(
            builder: (context, state) {
              if (state.status == NewProjectsStatus.initial || state.status == NewProjectsStatus.loading) {
      ...
      return Container(
        ...
        child: BlocBuilder<AuthBloc, AuthState>(
          builder: (context, authState) {
            ...
            // Optional: detect if we are currently on Fav_Logout screen
            // (so filled heart only when actually on favorites page)
            ...
            return Row(
              ...
              children: [
                ...
                // Favorites (heart) – now fully BLoC-reactive
                IconButton(
                  ...
                  onPressed: () {
                    if (isLoggedIn) {
                      ...
                      // Show login dialog (you can also move this to AuthBloc later)
                      showDialog(
                        context: context,
                  ...
                  icon: Icon(
                    // Filled heart only when logged in AND (optionally) on favorites page
                    (isLoggedIn && isOnFavorites)
                        ? Icons.favorite
