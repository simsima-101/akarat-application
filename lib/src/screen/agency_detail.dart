...
class _Agency_DetailState extends State<Agency_Detail> {
  ...
  Future<void> _callValidateListing({
    ...
    try {
      ...
      ).timeout(const Duration(seconds: 25)); // slightly increased from 8s →
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
            '❌ validate-listing failed: ${response.statusCode} ${response.body}');
      }
    } catch (e) {
  ...
  Future<void> fetchProducts(String data) async {
    ...
    if (now - lastFetched < const Duration(hours: 6).inMilliseconds) {
      ...
      if (cachedData != null) {
        ...
        final permitNumber =
            (cachedModel.data?.regulatoryInfo?.dldPermitNumber ?? '')
                .toString()
                .trim();
        ...
        if (permitNumber.isNotEmpty && listingId.isNotEmpty) {
          _callValidateListing(
              permitNumber: permitNumber, listingId: listingId);
        }
        return;
      }
    }
    ...
    try {
      ...
      final response = await ApiService.get(
        'properties/$data',
          ...
          seconds: 25)); // increased to 15s (6s too short for property details)
      ...
      if (response.statusCode == 200) {
        ...
        try {
          final permitNumber =
              (parsedModel.data?.regulatoryInfo?.dldPermitNumber ?? '')
                  .toString()
                  .trim();
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: CachedNetworkImage(
                      imageUrl:
                          productModels!.data!.agentImage?.isNotEmpty == true
                  ...
                  Row(
                    children: const [
                      ...
                      Padding(
                          padding: EdgeInsets.only(top: 5, left: 63),
                          child: Text("17")),
                    ],
                  ),
                  GestureDetector(
