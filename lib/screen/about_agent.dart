              ...
              child: TabBarView(
                children: [
                  SingleChildScrollView(
                      child: AboutTab(agentDetail: agentDetail)),
                  SingleChildScrollView(
                      child: PropertiesTab(agentProperties: agentProperties)),
...
// Dummy Tabs for separation
    class AboutTab extends StatelessWidget {
      final dynamic agentDetail;

      const AboutTab({super.key, required this.agentDetail});
    ...
    class PropertiesTab extends StatefulWidget {
      final dynamic agentProperties;

       const PropertiesTab({super.key, required this.agentProperties});
          ...
          child: Column(
            children: [
              ...
              ListView.builder(
                ...
                itemCount: widget.agentProperties?.data?.length ?? 0,
                shrinkWrap: true,
                itemBuilder: (context, index) {
                  if(widget.agentProperties== null){
                    return Scaffold(
                      body: Center(child: CircularProgressIndicator()), // Show loading state
                    );
                  }
                  bool isFavorited = favoriteProperties.contains(widget.agentProperties!.data![index].id);
                  return SingleChildScrollView(
                      child: GestureDetector(
                        onTap: (){
                          String id = widget.agentProperties!.data![index].id.toString();
                                              ...
                                              children: [
                                                AspectRatio(
                                                  ...
                                                  child: ListView.builder(
                                                    scrollDirection: Axis.horizontal,
                                                    physics: const ScrollPhysics(),
                                                    itemCount: widget.agentProperties?.data?[index].media?.length ?? 0,
                                                    shrinkWrap: true,
                                                    itemBuilder: (BuildContext context, int index) {
                                                      return CachedNetworkImage( // this is to fetch the image
                                                        imageUrl: (widget.agentProperties!.data![index].media![index].originalUrl.toString()),
                                                        fit: BoxFit.fill,
                                                      );
                                                    },
                                                  ),
                                  ...
                                  Padding(padding: const EdgeInsets.only(top: 5),
                                    child: ListTile(
                                      title: Padding(
                                        ...
                                        child: Text(widget.agentProperties!.data![index].title.toString(),
                                          style: TextStyle(
                                              fontSize: 16,height: 1.4
                                          ),),
                                      ),
                                      subtitle: Text('${widget.agentProperties!.data![index].price} AED',
                                        style: TextStyle(
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      ...
                                      Padding(padding: const EdgeInsets.only(left: 0,right: 0,top: 0),
                                        child: Text(widget.agentProperties!.data![index].location.toString(),style: TextStyle(
                                    ...
                                    children: [
                                      Padding(padding: const EdgeInsets.only(left: 30,top: 20,bottom: 15),
                                        child: ElevatedButton.icon(
                                            onPressed: () async {
                                              String phone = 'tel:${widget.agentProperties!.data![index].id}';
                                              try {
                                                final bool launched = await launchUrlString(
                                                  phone,
                                                  mode: LaunchMode.externalApplication, // ✅ Force external
                                      ...
                                      Padding(padding: const EdgeInsets.only(left: 15,top: 20,bottom: 15),
                                        child: ElevatedButton.icon(
                                            onPressed: () async {
                                              final phone = widget.agentProperties!.data![index]; // without plus
                                              final message = Uri.encodeComponent("Hello");
                                              // final url = Uri.parse("https://api.whatsapp.com/send/?phone=971503440250&text=Hello");
                                              // final url = Uri.parse("https://wa.me/?text=hello");
                                              final url = Uri.parse("https://api.whatsapp.com/send/?phone=%2B$phone&text&type=phone_number&app_absent=0");
                      ...
                      child: Column(
                        children: [
                          ...
                          Padding(padding: const EdgeInsets.only(
                            ...
                            child: Text(
                                "In the realm of real estate, Ben Caballero's name"
