...
class FliterListDemo extends StatefulWidget {
  ...
  final String? location;
  final String selectedPurpose;       // "Buy" | "Rent" | "New Projects"
  final String selectedPropertyType;  // e.g. "Apartment" | "" (any)
...
// WhatsApp sanitizer: always outputs 971XXXXXXXXX (no plus)
  ...
  Future<void> _onCreateAlert() async {
    ...
    final initialPurpose =
    purpose.isEmpty ? selectedPurposeText : purpose;

    final initialPropertyType =
    (property_type.trim().isEmpty || property_type.toLowerCase() == 'all residential')
        ? ''   // '' = Any type for API
        : property_type.trim();

    final availableTypes = (propertyTypeModel?.data ?? const [])
        .map((e) => (e.name ?? '').trim())   // prefer e.slug if your API expects slugs
  ...
  void _resetAllFilters() {
    ...
    bedroom = ' ';
    bathroom = ' ';
    property_type = 'All Residential'; // neutral UI label, stripped in query
    purpose = '';                      // neutral (no filter)
  }
...
// int _perPage = 40;  // or whatever you want to request on first load
  ...
  String buildApiUrl() {
    ...
    final effectivePurpose = (purpose.isNotEmpty ? purpose : selectedPurposeText);
...
// Seed from caller (FliterList → FliterListDemo)
      selectedPurposeText = (widget.selectedPurpose.isNotEmpty ? widget.selectedPurpose : 'Rent');
// Use the same label for the actual filter until user changes it in UI
      purpose = selectedPurposeText;

// If caller left it empty or "All Residential", keep the neutral UI label;
// buildApiUrl() already strips this to '' for the query param.
      property_type = (widget.selectedPropertyType.isNotEmpty
...
// adjust to your item model type
  ...
  // ✅ Environment-safe, provider-driven toggle.
// Uses whatever API_BASE_URL your build was launched with.
// Assumes FavoriteProvider has: Future<bool> toggleFavoriteWithApi(int id, String token, BuildContext ctx)
  ...
  Future<bool> toggledApi(BuildContext context, String token, int propertyId) async {
    ...
    try {
      final ok = await context
          ...
          .toggleFavoriteWithApi(propertyId, token, context); // 👈 single source of truth
                                  ...
                                  child: InkWell(
                                    ...
                                    onTap: () async {
                                      final result = await Navigator.push(
                                        context,
                                        MaterialPageRoute(
                                          builder: (context) => Filter(data: (purpose.isEmpty ? selectedPurposeText : purpose)),
...
// reset selection only if you want to neutralize cross-purpose types
                                                      setState(() {
                                                        selectedtype = null;
                                                        property_type = 'All Residential'; // UI label only; query removes it
                                                      });
                          ...
                          GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: Container(
                                ...
                                child: Center(
                                  child: Text(
                                    (property_type.trim().isEmpty ? 'All Residential' : property_type.trim()),
                                    style: const TextStyle(
                                      fontWeight: FontWeight.bold,
                                      fontSize: 14,
                                    ),
                                              ...
                                              builder: (context, favProvider, _) {
                                                ...
                                                return IconButton(
                                                    ...
                                                    onPressed: () async {
                                                      ...
                                                      if (propertyId == null) {
                                                        debugPrint('⚠️ property.id is null; cannot toggle favorite.');
                                                        return;
                                                      }

                                                      // ✅ Use Provider's API-integrated method (optimistic + revert handled inside)
                                                      final success = await favProvider.toggleFavoriteWithApi(propertyId, token, context);
